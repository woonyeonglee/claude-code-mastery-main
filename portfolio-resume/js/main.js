...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Hero 섹션 다중 텍스트 순환 타이핑 효과
    const heroTyping = document.getElementById('hero-typing');
    if (heroTyping) {
        const titles = ['Software Developer', 'Frontend Engineer', 'Web Developer', 'UI/UX Enthusiast'];
        let titleIndex = 0;
        let charIndex = 0;
        ...
        function typeHero() {
            ...
            if (isDeleting) {
                heroTyping.textContent = currentTitle.substring(0, charIndex - 1);
                charIndex--;
            } else {
                heroTyping.textContent = currentTitle.substring(0, charIndex + 1);
                charIndex++;
    ...
    function openModal(projectItem) {
        const { title, desc, tech, github, demo, img } = projectItem.dataset;
        ...
        tech.split(',').forEach(t => {
            ...
            span.className = 'px-3 py-1 bg-accent-orange/10 text-accent-orange border border-accent-orange/30 rounded-full text-sm';
    ...
    if (projectGrid) {
        projectGrid.addEventListener('click', (e) => {
            ...
            // GitHub/데모 링크 클릭 시에는 모달을 열지 않음
            if (projectItem && !e.target.closest('a')) {
    ...
    if (contactForm) {
        ...
        // 유효성 검사 규칙
        const validators = {
            name: (v) => v.trim().length >= 2 ? '' : '이름은 2자 이상 입력해주세요.',
            email: (v) => emailRegex.test(v.trim()) ? '' : '올바른 이메일 형식을 입력해주세요.',
            subject: (v) => v.trim().length >= 1 ? '' : '제목을 입력해주세요.',
            message: (v) => v.trim().length >= 10 ? '' : '메시지는 10자 이상 입력해주세요.',
        };
        ...
        function showFieldError(fieldName, message) {
            ...
            if (message) {
                ...
                inputEl.classList.remove('border-dark-border');
            } else {
                errorEl.classList.add('hidden');
                inputEl.classList.remove('border-red-400');
                inputEl.classList.add('border-dark-border');
            }
        }

        // 실시간 유효성 검사 (input 이벤트)
    ...
    // 프로젝트 필터 버튼 화살표 키 네비게이션
    const filterContainer = document.querySelector('#projects .flex.flex-wrap.justify-center.gap-4');
    if (filterContainer) {
        ...
        filterContainer.addEventListener('keydown', (e) => {
            ...
            const btns = Array.from(filterContainer.querySelectorAll('.filter-btn'));
